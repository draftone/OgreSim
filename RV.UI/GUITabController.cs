...
public class GUITabController : MonoBehaviour
{
    ...
    public struct TabContentsPair
    {
        ...
        public void OnEnableTabInteractable(bool b)
        {
            tab.interactable = b;
            contentBody.gameObject.SetActive(!b);
            if (optionals.selectedTab != null)
            {
                optionals.selectedTab.gameObject.SetActive(b);
                tab.gameObject.SetActive(b);
    ...
    void OnEnable()
    {
        if (tabContentsPairs != null)
        {
            ...
            foreach (TabContentsPair obj in tabContentsPairs)
            {
                obj.OnEnableTabInteractable(true);
            }
            tabContentsPairs[0].OnEnableTabInteractable(false);
        }
    }
}